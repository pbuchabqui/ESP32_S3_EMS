...
esp_err_t sync_process_tooth(uint32_t tooth_time) {
    ...
    sync_update_from_capture((uint64_t)tooth_time, false, true);
    return ESP_OK;
}
...
esp_err_t sync_detect_phase(void) {
    ...
    portENTER_CRITICAL(&g_sync_spinlock);
        // Phase detection logic for 60-2 wheel
        // Phase is detected when we see the gap after tooth 58
        if (g_sync_data.gap_detected && g_sync_data.tooth_index == 0) {
            g_sync_data.phase_detected = true;
            g_sync_data.phase_detected_time = g_sync_data.last_tooth_time;
            ESP_LOGI("SYNC", "Phase detected at time %d", g_sync_data.phase_detected_time);
        }
...
esp_err_t sync_get_data(sync_data_t *data) {
    ...
    portEXIT_CRITICAL(&g_sync_spinlock);
    data->latency_us = (uint32_t)(esp_timer_get_time() - data->last_update_time);
    return ESP_OK;
}
...
static void sync_update_from_capture(uint64_t capture_us, bool from_isr, bool emit_log) {
    ...
    if (total_positions > 0) {
        g_sync_data.time_per_degree = (g_sync_data.tooth_period * total_positions) / 360;
    }
    ...
    uint32_t age_us = (uint32_t)(esp_timer_get_time() - g_sync_data.last_update_time);
    g_sync_data.sync_valid = (g_sync_data.rpm > 0) && (age_us < 200000);
...
static void IRAM_ATTR sync_cmp_gpio_isr(void *arg) {
    (void)arg;
    if (!g_hw_sync_enabled || g_cmp_gptimer == NULL) {
        return;
    }
...
static bool IRAM_ATTR sync_pcnt_on_reach(pcnt_unit_handle_t unit,
    ...
    if (!g_hw_sync_enabled || g_sync_gptimer == NULL) {
        return false;
    }
