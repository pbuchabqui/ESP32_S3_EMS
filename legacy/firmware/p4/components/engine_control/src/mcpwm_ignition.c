...
bool mcpwm_ignition_init(void) {
    ...
    const gpio_num_t gpios[4] = {
        GPIO_NUM_25, GPIO_NUM_26, GPIO_NUM_27, GPIO_NUM_28
    };
    ...
    for (int i = 0; i < 4; i++) {
        ...
        ESP_ERROR_CHECK_WITHOUT_ABORT(mcpwm_new_comparator(g_channels[i].oper, &cmp_cfg, &g_channels[i].cmp_dwell));
        ESP_ERROR_CHECK_WITHOUT_ABORT(mcpwm_new_comparator(g_channels[i].oper, &cmp_cfg, &g_channels[i].cmp_spark));
        ...
        ESP_ERROR_CHECK_WITHOUT_ABORT(mcpwm_generator_set_action_on_timer_event(
            g_channels[i].gen,
...
bool mcpwm_ignition_start_cylinder(uint8_t cylinder_id, uint16_t rpm, float advance_degrees, float battery_voltage) {
    ...
    if (mcpwm_comparator_set_compare_value(ch->cmp_dwell, dwell_start_delay) != ESP_OK) {
        return false;
    }
    if (mcpwm_comparator_set_compare_value(ch->cmp_spark, spark_ticks) != ESP_OK) {
        return false;
    }
...
bool mcpwm_ignition_schedule_one_shot(uint8_t cylinder_id, uint32_t spark_delay_us, uint16_t rpm, float battery_voltage) {
    ...
    uint32_t period = spark_delay_us + 1;
    if (period <= spark_delay_us) {
        period = spark_delay_us + 1;
    }
    ...
    if (mcpwm_comparator_set_compare_value(ch->cmp_spark, spark_delay_us) != ESP_OK) {
        return false;
    }
