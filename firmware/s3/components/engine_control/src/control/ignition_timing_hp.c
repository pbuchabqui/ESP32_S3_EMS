...
bool ignition_hp_init(void) {
    // Inicializar sistemas de alta precisão
    hp_init_phase_predictor(&g_phase_predictor, 10000.0f);  // 10ms inicial
    ...
    if (ign_ok && inj_ok) {
        ...
        LOG_IGNITION_I("  Phase predictor: active");
        LOG_IGNITION_I("  Hardware latency compensation: active");
        LOG_IGNITION_I("  Jitter measurement: active");
        return true;
    }
...
void ignition_hp_apply_timing(uint16_t advance_deg10, uint16_t rpm) {
    ...
    if (have_sync) {
        ...
        for (uint8_t cylinder = 1; cylinder <= 4; cylinder++) {
            ...
            // Calcular delay usando predição de fase
            float predicted_period = hp_predict_next_period(&g_phase_predictor, 0);
            float base_delay = delta_deg * us_per_deg;
            
            // Aplicar compensação de latência física
            float compensated_delay = base_delay;
            mcpwm_ignition_hp_apply_latency_compensation(&compensated_delay, 
                                                         battery_voltage, 
                                                         (float)sensors.clt_c);
        ...
        // Atualizar preditor de fase
        float measured_period = sync_data.tooth_period;
        hp_update_phase_predictor(&g_phase_predictor, measured_period, hp_get_cycle_count());
        
        LOG_IGNITION_D("HP Scheduled ignition (sync): %u deg10, %u RPM", advance_deg10, rpm);
        return;
    }

    // Fallback: usar predição de fase
    float predicted_period = hp_predict_next_period(&g_phase_predictor, 0);
...
void ignition_hp_get_jitter_stats(float *avg_us, float *max_us, float *min_us) {
    mcpwm_ignition_hp_get_jitter_stats(avg_us, max_us, min_us);
}

void ignition_hp_update_phase(float measured_period_us) {
    hp_update_phase_predictor(&g_phase_predictor, measured_period_us, hp_get_cycle_count());
}
