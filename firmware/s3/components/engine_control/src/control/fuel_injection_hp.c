...
void fuel_injection_hp_init(const fuel_injection_config_t *config) {
    if (config) {
        g_fuel_cfg = *config;
    }
    // Drivers HP já inicializados em ignition_hp_init()
}

bool fuel_injection_hp_schedule_eoi_ex(uint8_t cylinder_id,
    ...
    // Aplicar compensação de latência do injetor
...
bool fuel_injection_hp_schedule_eoi(uint8_t cylinder_id,
                                   float target_eoi_deg,
                                   uint32_t pulsewidth_us,
                                   const sync_data_t *sync) {
    return fuel_injection_hp_schedule_eoi_ex(cylinder_id, target_eoi_deg, pulsewidth_us, sync, NULL);
}

bool fuel_injection_hp_schedule_sequential(uint32_t pulsewidth_us[4],
    ...
    for (int i = 0; i < 4; i++) {
        fuel_injection_schedule_info_t info;
        if (!fuel_injection_hp_schedule_eoi_ex(i + 1, target_eoi_deg[i], pulsewidth_us[i], sync, &info)) {
            return false;
        }
